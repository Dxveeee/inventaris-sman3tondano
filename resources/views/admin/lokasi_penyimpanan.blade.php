...
@include('layouts.sidebar')
    ...
    <main class="main">
        ...
        <div class="topbar">
            <div class="topbar-title">
                ...
                <p>Daftar lokasi penyimpanan inventaris SMA Negeri 3 Tondano</p>
            </div>
        ...
        <section class="content">
            ...
            <div class="content-header">
                <div class="content-title">
                    <h2>Daftar Lokasi</h2>
                    <p>Total {{ count($lokasi) }} lokasi terdaftar</p>
                </div>
            ...
            <div class="card">
                                        ...
                                        <td>
                                            <div class="action-cell">
                                                ...
                                                <button type="button" class="btn btn-edit"
                                                    onclick="openEditModal({{ $item->id }}, '{{ addslashes($item->nama) }}',  '{{ addslashes($item->deskripsi ?? '') }}')">
                                                    <i class="fas fa-pencil-alt"></i>
                                                </button>

                                                <!-- Tombol Hapus -->
                                                <form method="POST" action="{{ route('admin.lokasi.destroy', $item->id) }}" style="display: inline;">
                                                    @csrf
                                                    @method('DELETE')
                ...
                @else
                    <div class="empty-state">
                        ...
                        <p>Belum ada lokasi penyimpanan yang terdaftar</p>
                        <button type="button" class="btn btn-primary" onclick="openTambahModal()">
    ...
    <!-- MODAL TAMBAH LOKASI -->
    <div class="modal" id="tambahLokasiModal">
        <div class="modal-content">
            <div class="modal-header">
                <h3><i class="fas fa-map-marker-alt" style="color: var(--accent); margin-right: 6px;"></i>Tambah Lokasi Baru</h3>
            ...
            <form method="POST" action="{{ route('admin.lokasi.store') }}">
                ...
                <div class="form-group">
                    <label for="nama">Nama Lokasi <span style="color: var(--danger);">*</span></label>
                    <input type="text" id="nama" name="nama" required placeholder="Contoh: Ruang Kelas 10A">
                </div>

                <div class="form-group">
                    <label for="deskripsi">Deskripsi</label>
                    <textarea id="deskripsi" name="deskripsi" placeholder="Masukkan keterangan tambahan (opsional)"></textarea>
                </div>
    ...
    <!-- MODAL EDIT LOKASI -->
    <div class="modal" id="editLokasiModal">
        <div class="modal-content">
            <div class="modal-header">
                <h3><i class="fas fa-pencil-alt" style="color: var(--info); margin-right: 6px;"></i>Edit Lokasi</h3>
                <button type="button" class="modal-close" onclick="closeEditModal()">×</button>
            </div>

            <form method="POST" id="editLokasiForm" action="">
                @csrf
                @method('PUT')

                <div class="form-group">
                    <label for="edit_nama">Nama Lokasi <span style="color: var(--danger);">*</span></label>
                    <input type="text" id="edit_nama" name="nama" required placeholder="Contoh: Ruang Kelas 10A">
                </div>

                <div class="form-group">
                    <label for="edit_deskripsi">Deskripsi</label>
                    <textarea id="edit_deskripsi" name="deskripsi" placeholder="Masukkan keterangan tambahan (opsional)"></textarea>
                </div>
    ...
    <script>
        ...
        function openTambahModal() {
            document.getElementById('tambahLokasiModal').classList.add('active');
        }
        function closeTambahModal() {
            document.getElementById('tambahLokasiModal').classList.remove('active');
        }
        ...
        function openEditModal(id, nama, deskripsi) {
            ...
            document.getElementById('editLokasiForm').action = '/admin/lokasi/' + id;
            document.getElementById('editLokasiModal').classList.add('active');
        }
        function closeEditModal() {
            document.getElementById('editLokasiModal').classList.remove('active');
        }

        // Tutup modal saat klik di luar
        document.getElementById('tambahLokasiModal').addEventListener('click', (e) => {
            if (e.target.id === 'tambahLokasiModal') closeTambahModal();
        });
        document.getElementById('editLokasiModal').addEventListener('click', (e) => {
            if (e.target.id === 'editLokasiModal') closeEditModal();
        });
