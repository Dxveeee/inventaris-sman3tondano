...
<head>
    ...
    <title>Pilih Jenis Barang</title>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .item-icon { width: 46px; height: 46px; border-radius: 16px; background: rgba(255,152,0,.12); display: inline-flex; align-items: center; justify-content: center; color: var(--accent); margin-bottom: 18px; font-size: 18px; }
        .item-grid {
            display: grid;
            gap: 18px;
        ...
        .item-card { background: #fff; border: 1px solid var(--border); border-radius: 16px; padding: 22px; transition: transform .18s, box-shadow .18s; text-decoration: none; color: var(--text); }
        .item-card:hover { transform: translateY(-2px); box-shadow: 0 10px 22px rgba(0,0,0,.08); }
        .item-title { font-size: 15px; font-weight: 700; }
        .item-meta { display: grid; gap: 10px; font-size: 13px; color: var(--muted); margin-bottom: 16px; }
        .item-meta span { display: block; margin-top: 8px; }
        .item-meta .badge { display: inline-flex; align-items: center; justify-content: center; padding: 5px 10px; border-radius: 999px; font-size: 12px; font-weight: 700; color: #fff; }
        .item-meta .badge-baik { background: var(--success); }
        .item-meta .badge-rusak_ringan { background: var(--warning); }
        .item-meta .badge-rusak_berat { background: var(--danger); }
        .item-meta .badge-info { background: var(--info); }
        ...
        .badge-info { background: var(--info); margin-top: 8px; }
...
<div class="main">
    <div class="topbar">
        <div class="topbar-title">
            <h1>Pilih Jenis Barang</h1>
            <p>Pilih jenis barang yang tersedia di lokasi ini.</p>
        </div>
    ...
    <div class="content-card">
        ...
        <a href="{{ route('peminjam.barang.index') }}" class="back-button">
            <i class="fas fa-arrow-left"></i>
            Kembali
        </a>

        @if($barangList->isEmpty())
            <div class="empty-state">Tidak ada jenis barang tersedia pada lokasi ini.</div>
        @else
            <div class="item-grid">
                @foreach($barangList as $item)
                    <a href="{{ route('peminjam.barang.pilihUnit', ['idLokasi' => $lokasi->id, 'namaBarang' => $item->nama_barang]) }}" class="item-card">
                        <div class="item-icon"><i class="fas fa-box"></i></div>
                        <div class="item-title">{{ $item->nama_barang }}</div>
                        <span class="badge badge-info">{{ $item->jumlah_tersedia }} unit tersedia</span>
                    </a>
                @endforeach
            </div>
        @endif
